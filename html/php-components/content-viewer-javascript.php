...
<script>
    ...
    <?php if ($_vPageContentEditMode) { ?>
        ...
        var questMarkdownEditorInstance = null;

        function getQuestMarkdownEditor()
        {
            if (!questMarkdownEditorInstance && window.MarkdownEditor) {
                questMarkdownEditorInstance = window.MarkdownEditor.create({
                    textareaId: 'content-edit-markdown-textbox',
                    previewId: 'content-edit-markdown-preview',
                    writeToggleId: 'markdown-view-toggle-code',
                    previewToggleId: 'markdown-view-toggle-preview',
                });
            }

            return questMarkdownEditorInstance;
        }

        function HandleMarkdownEditorInput()
        {
            var editor = getQuestMarkdownEditor();
            if (editor) {
                editor.handleInput();
            }
        }
        ...
        function ApplyMarkdownWrap(prefix, suffix, placeholder = "")
        {
            var editor = getQuestMarkdownEditor();
            if (editor) {
                editor.applyWrap(prefix, suffix, placeholder);
            }
        }
        ...
        function ApplyMarkdownBlock(prefix, suffix, placeholder = "")
        {
            var editor = getQuestMarkdownEditor();
            if (editor) {
                editor.applyBlock(prefix, suffix, placeholder);
            }
        }
