...
class TransactionController
{
    ...
    public static function markTransactionAsComplete(vTransaction $transaction) : void
    {
        $sql = "UPDATE `transaction` SET complete = 1 WHERE ctime = ? AND crand = ? LIMIT 1;";
        $params = [$transaction->ctime, $transaction->crand];

        try
        {
            $result = Database::executeSqlQuery($sql, $params);

            if(!$result) throw new Exception("Failed to mark transaction as complete");
        }
        catch(Exception $e)
        {
            throw new Exception("Exception caught while marking transaction as complete : $e");
        }
    }

    public static function markTransactionAsVoid(vTransaction $transaction) : void
    {
        $sql = "UPDATE `transaction` SET void = 1 WHERE ctime = ? AND crand = ? LIMIT 1;";
        $params = [$transaction->ctime, $transaction->crand];
        ...
        try
        {
            ...
            if(!$result) throw new Exception("Failed to mark transaction as void");
        }
        catch(Exception $e)
        {
            throw new Exception("Exception caught while marking transaction as void : $e");
        }
    }
    ...
    public static function getTransactionBetweenAccounts(vRecordId $firstAccount, vRecordId $secondAccount) : Response
    {
        ...
        try
        {
            ...
            $transactions = static::rowsTransactionComponentsWithHeaderInformationToView($result);
    ...
    public static function getTransactionById(vRecordId $transactionId) : Response
    {
        $resp = new Response(false, "unkown error in getting transaction by id", null);
    ...
    private static function rowsTransactionComponentsWithHeaderInformationToView(mysqli_result $result) : array
    {
        $transactions = [];
