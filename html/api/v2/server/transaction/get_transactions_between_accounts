...
use Kickback\Services\TransactionService;
...
try
{
    ...
    $response = null;
    $response_code = TransactionService::get_transactions_between_accounts($sessionAccount, $request_contents_json, $response);
    if ( $response_code !== 0 ) {
...
catch( \Throwable $e )
{
    ...
    $response = new Response(false,
        'Failed to retrieve transactions between accounts: ' . ThrowableOverrides::message($e),
        $e->__toString());
}
finally {
    Endpoint::end();
